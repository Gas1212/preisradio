...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    console.log(`✓ Generated sitemap with ${allUrls.length} URLs (${staticPages.length} static, ${brandPages.length} brands, ${categoryPages.length} categories, ${productPages.length} products)`);
    return allUrls;
  } catch (error) {
